...
export default class Pagination extends Component {
  ...
  static getDerivedStateFromProps(prevProps, nextState) {
    if (nextState.currentPage > 1) {
      // Moi lan chuyen trang
      nextState.startIndex = nextState.currentPage - 2;
      nextState.endIndex = nextState.currentPage + 1;
    } else if (nextState.currentPage === 1) {
      //Truong hop chuyen ve first page
      nextState.startIndex = 0;
      nextState.endIndex = 3;
    } else if (nextState.currentPage === nextState.pageQuantity) {
      //Truong hop chuyen den last page
  ...
  render() {
    ...
    const { JSXitems } = this.props; // Mang JSX truyen tu SearchContainer
    ...
    return (
      <section className="pagi">
        <ul className="pagi__wrapper">
          <li
            className={`pagi__item ${
              this.state.currentPage === pageNumbers[0] ? "disabled" : ""
            }`}
          ...
          {this.state.currentPage >= 3 ? (
            <>
              <li className={`pagi__item`} onClick={this.handleBackFirstPage}>
                1
              </li>
          ...
          <li
            className={`pagi__item ${
              this.state.currentPage === pageNumbers.length ? "active" : ""
            }`}
